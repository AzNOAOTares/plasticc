...
def make_index_for_release(data_release, data_dir=None, redo=False):
    ...
    for header_file in files:
        ...
        fieldname, modelname = dirname.replace('LSST_', '').replace('IDEAL_','').split('_')
        modelname = modelname.replace('MODEL','')
        basename = basename.replace('LSST_','').replace('IDEAL_','').replace(fieldname+'_','').replace('_HEAD.FITS.gz','').replace('_HEAD.FITS','')
